...
public class UserServiceImpl implements UserService {
    ...
    public boolean register(User user) {
        ...
        if (userDao.findByEmail(user.getEmail()).isPresent()) {
            ...
            throw new UserAlreadyRegisteredRuntimeException("User is already registered by this e-mail");
        }
    ...
    public User login(String email, String password) {
        ...
        if (!entity.isPresent()) {
            ...
            throw new UserNotExistRuntimeException("There is no servlet with this e-mail");
        } else {
            if (entity.get().getPassword().equals(encodedPassword)) {
                return mapper.mapUserEntityToUser(entity.get());
            } else {
                LOGGER.warn("Incorrect password");
                throw new UserNotExistRuntimeException("Incorrect password");
            }
        }
    }
