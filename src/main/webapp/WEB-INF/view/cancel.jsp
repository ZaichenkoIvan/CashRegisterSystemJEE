...
<c:if test="${sessionScope.user.idUserType == '2'}">
    <FORM action="cancel" method="POST">
        <table>
            <tr>
                ...
                <td><input name="checkid" type="number" min="1" placeholder="<fmt:message key="cancel.search"/>"
                           autofocus required/></td>
            </tr>
            <tr>
                <td><input type="submit" class="button" name="btnSearchCheck" value=
                    <fmt:message key="cancel.find"/>/></td>
            </tr>
        </table>
    </FORM>
    <c:if test="${sessionScope.check.canceled == '1'}"><p><fmt:message key="check.check"/> № ${sessionScope.check.id}
        <fmt:message key="cancel.canceled"/>!</p></c:if>
    ...
    <c:if test="${sessionScope.check.canceled == '0' and not empty checkspecs}">
        ...
        <FORM action="cancel" method="POST">
            <table>
                <tr>
                    <td><input type="submit" class="button" name="btnCancelCheck" value=
                        <fmt:message key="cancel.check"/>/></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td><input type="submit" class="button" placeholder="Поиск по № спецификации"
                               name="btnCancelCheckspec"
                               value=
                                   <fmt:message key="cancel.btnitem"/>/>
                    </td>
                </tr>
            </table>
        </FORM>
    ...
    <FORM action="cancel" method="POST">
        <table>
            <tr>
                ...
                <td><input type="submit" class="button" name="btnXReport" value=
                    <fmt:message key="xreport"/>/></td>
            </tr>
            <tr>
                <td style="text-align: left"><fmt:message key="gen"/> <fmt:message key="zreport"/>:</td>
                <td><input type="submit" class="button" name="btnZReport" value=
                    <fmt:message key="zreport"/>/></td>
            </tr>
        </table>
    </FORM>
