...
<c:if test="${sessionScope.user.idUserType == '3' or sessionScope.user.idUserType == '2'}">
    <h4>
        ...
        <c:if test="${not empty goodsCodeNotFound}"><fmt:message key="check.code"/> ${goodsCodeNotFound} <fmt:message
                key="check.codenotfound"/>!</c:if>
        <c:if test="${not empty goodsNameNotFound}"><fmt:message key="check.goodname"/> ${goodsNameNotFound}
            <fmt:message key="check.namenotfound"/>!</c:if>
    </h4>
    <FORM method="POST" name="addcheck">
        <table>
            ...
            <tr>
                <td><input type="submit" class="button" name="btnAddCheckspec" value=
                    <fmt:message key="check.addgood"/>/></td>
            </tr>
        </table>
    </FORM>
    <FORM method="POST">
        <table>
            <tr>
                <td><input type="submit" class="button" name="btnCreateCheck" value=
                    <fmt:message key="check.createcheck"/>/></td>
            </tr>
        </table>
    </FORM>
