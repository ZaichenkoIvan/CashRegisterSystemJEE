CREATE DATABASE projectcash;

CREATE TABLE projectcash.user_roles (
  role_id INT NOT NULL KEY,
  role_description VARCHAR(32) UNIQUE
) ENGINE InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE projectcash.invoice_status (
  status_id INT NOT NULL KEY,
  status_description VARCHAR(32) UNIQUE
) ENGINE InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE projectcash.products (
...
CREATE TABLE projectcash.users (
  ...
  FOREIGN KEY (role_id) REFERENCES projectcash.user_roles (role_id)
) ENGINE InnoDB DEFAULT CHARSET=utf8;



CREATE TABLE projectcash.invoices (
  ...
  FOREIGN KEY (user_name) REFERENCES projectcash.users (user_name)
    ON UPDATE CASCADE
    ON DELETE SET NULL,
  FOREIGN KEY (status_id) REFERENCES projectcash.invoice_status(status_id)
) ENGINE InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE projectcash.payments (
  ...
  FOREIGN KEY (product_code) REFERENCES projectcash.products(product_code)
    ON UPDATE CASCADE
    ON DELETE RESTRICT,
  FOREIGN KEY (invoice_code) REFERENCES projectcash.invoices(invoice_code)
    ON UPDATE CASCADE
    ON DELETE RESTRICT,
  FOREIGN KEY (status_id) REFERENCES projectcash.invoice_status(status_id)
) ENGINE InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE projectcash.transactions (
  transaction_id INT NOT NULL AUTO_INCREMENT KEY,
